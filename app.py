...
model = pickle.load(open('./best_models/rf.pkl','rb'))
...
def predict():
    ...
    probas = {}
    for g, p in zip(model.classes_, probs):
        probas[g] = p.round(4)
    
    probability = probas[output] * 100

    # makes the prediction of the inputs passed in with the model. 
    return render_template("home.html", prediction_test=output, probability=probability)
